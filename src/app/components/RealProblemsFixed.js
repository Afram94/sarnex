...
export default function RealProblemsFixed() {
  ...
  useEffect(() => {
    ...
    axios.get('http://localhost:8000/api/problem-section')
      .then(res => setSectionStyle(res.data))
      .catch(err => console.error('Error loading section:', err))
  }, [])

  if (!sectionStyle || problems.length === 0) return null

  return (
    <section
      className={`relative py-28 px-6 overflow-hidden font-${sectionStyle.font_family || 'sans'}`}
      style={{ backgroundColor: sectionStyle.section_bg_color || '#0d0d0d', color: '#fff' }}
    >
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_50%_50%,#9cc0ab22_0%,transparent_60%)]" />

      <div className="relative z-10 max-w-4xl mx-auto text-center">
        <h2 className="text-4xl sm:text-5xl font-bold text-[#9cc0ab]">
          {sectionStyle.title || 'You Deserve Better'}
        </h2>
        <p className="mt-6 text-white/70 text-lg">
          {sectionStyle.subtitle || 'Real problems small businesses face — solved with modern web design.'}
        </p>
      </div>
      ...
      <div className="relative z-10 mt-20 max-w-6xl mx-auto grid grid-cols-1 sm:grid-cols-2 gap-12">
        {problems.map(({ id, before, after, card_bg_color, text_color, font_family }, index) => (
          <motion.div
            ...
            viewport={{ once: true, amount: 0.3 }}
            transition={{ duration: 0.6, delay: index * 0.1 }}
            className={`rounded-3xl p-8 shadow-xl border border-white/10 transition-all duration-300 hover:shadow-[#9cc0ab]/20 font-${font_family || 'sans'} hover:shadow-[${text_color || '#9cc0ab'}]/20`}
            style={{
